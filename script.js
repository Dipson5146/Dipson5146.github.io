// Main script: typing + UI + small apps (weather, tasks, blog, snake, calc)
(() => {
  ...
  /* Typing animation (restored, removed unwanted words) */
  ...
  (function typeTick(){
    ...
    if(!deleting){
      if(ci < w.length) { ci++; setTimeout(typeTick, 140); }
      else { deleting = true; setTimeout(typeTick, 900); }
    } else {
      if(ci > 0){ ci--; setTimeout(typeTick, 80); }
      else { deleting = false; wi = (wi+1)%words.length; setTimeout(typeTick, 200); }
    }
  })();
  ...
  /* Section reveal highlight (simple) */
  const panels = $$('.panel');
  ...
  const projects = $$('.project'), modals = $$('.modal');
  ...
  taskList?.addEventListener('click', e => {
    const i = e.target.dataset.i; if(!i && i !== 0) return;
    const arr = loadTasks();
  ...
  function makeCalc(){
    ...
    calcGrid.innerHTML = '';
    buttons.forEach(b=>{
      const btn = document.createElement('button'); btn.className='calc-btn'; if('+-*/'.includes(b)) btn.classList.add('op'); btn.textContent=b; btn.dataset.val=b;
      calcGrid.appendChild(btn); btn.addEventListener('click', ()=> handleCalc(b));
    });
  ...
  function handleCalc(key){
    if(key==='='){ try{ expr = String(safeEval(expr)); } catch(e){ expr='ERR'; } updateCalc(); return; }
    expr += key; updateCalc();
  }
