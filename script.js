// script.js — typing + animated project entrance + modals + small apps
(() => {
  ...
  const typingWords = ['Learner', 'Web Developer', 'Designer'];
  if (typingEl) {
    let wIndex = 0, cIndex = 0, deleting = false;
    (function tick() {
      const word = typingWords[wIndex];
      typingEl.textContent = word.slice(0, cIndex);
      if (!deleting) {
        if (cIndex < word.length) { cIndex++; setTimeout(tick, 140); }
        else { deleting = true; setTimeout(tick, 900); }
      } else {
        if (cIndex > 0) { cIndex--; setTimeout(tick, 80); }
        else { deleting = false; wIndex = (wIndex + 1) % typingWords.length; setTimeout(tick, 200); }
      }
    })();
  }
  ...
  $$('nav a').forEach(a => a.addEventListener('click', () => navList.classList.remove('open')));

  /* Project entrance animation: IntersectionObserver with stagger */
  ...
  if ('IntersectionObserver' in window && projectEls.length) {
    const obs = new IntersectionObserver((entries, observer) => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting) {
          // compute index for stagger
          const index = projectEls.indexOf(el);
          // set a small CSS variable delay (ms)
    ...
    projectEls.forEach(p => obs.observe(p));
  } else {
    // fallback: reveal immediately
    projectEls.forEach((p, i) => { p.style.setProperty('--delay', `${i * 80}ms`); p.classList.add('in-view'); });
  }

  /* Generic modal open/close with focus trap */
  const projects = projectEls;
  const modals = $$('.modal');
  function focusables(root) {
    return Array.from(root.querySelectorAll('button, a, input, textarea, select, [tabindex]:not([tabindex="-1"])'))
  ...
  const TASKS_KEY = 'dipson_tasks_v1';
  ...
  function renderTasks() {
    const tasks = loadTasks(); if(!taskList) return; taskList.innerHTML = '';
  ...
  taskList?.addEventListener('click', e => {
    ...
    const arr = loadTasks();
    if (e.target.classList.contains('del')) { arr.splice(i, 1); saveTasks(arr); renderTasks(); }
    if (e.target.classList.contains('edit')) { const val = prompt('Edit task', arr[i].title); if (val !== null) { arr[i].title = val; saveTasks(arr); renderTasks(); } }
  });
  renderTasks();
  ...
  function renderPosts() {
    if(!postsEl) return;
    const posts = loadPosts(); postsEl.innerHTML = '';
  ...
  /* SNAKE */
  const canvas = $('#snakeCanvas'), ctx = canvas?.getContext('2d');
  let grid = 18, snake = [], dir = { x: 1, y: 0 }, food = null, score = 0, running = false, tick = 0, speed = 10;
  const snakeScore = $('#snakeScore');
  function initSnake() { if(!canvas) return; const cols = Math.floor(canvas.width / grid); snake = []; for (let i = 0; i < 5; i++) snake.push({ x: Math.floor(cols / 2) - i, y: Math.floor(cols / 2) }); spawnFood(); score = 0; if (snakeScore) snakeScore.textContent = score; dir = { x: 1, y: 0 }; }
  ...
  function drawSnake() { if(!ctx) return; ctx.fillStyle = '#0b1220'; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = '#ff4d6d'; ctx.fillRect(food.x * grid, food.y * grid, grid - 1, grid - 1); ctx.fillStyle = '#22c55e'; snake.forEach(s => ctx.fillRect(s.x * grid, s.y * grid, grid - 1, grid - 1)); }
  function stepSnake() { if (!running) return; tick++; if (tick % Math.max(1, Math.floor(20 - speed)) !== 0) { requestAnimationFrame(stepSnake); return; } const head = { x: snake[0].x + dir.x, y: snake[0].y + dir.y }; const cols = Math.floor(canvas.width / grid); head.x = (head.x + cols) % cols; head.y = (head.y + cols) % cols; if (snake.some(s => s.x === head.x && s.y === head.y)) { running = false; alert('Game over — score: ' + score); return; } snake.unshift(head); if (head.x === food.x && head.y === food.y) { score++; if (snakeScore) snakeScore.textContent = score; spawnFood(); } else snake.pop(); drawSnake(); requestAnimationFrame(stepSnake); }
  ...
  window.addEventListener('keydown', e => { if (!['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) return; if (e.key === 'ArrowUp' && dir.y !== 1) dir = { x: 0, y: -1 }; if (e.key === 'ArrowDown' && dir.y !== -1) dir = { x: 0, y: 1 }; if (e.key === 'ArrowLeft' && dir.x !== 1) dir = { x: -1, y: 0 }; if (e.key === 'ArrowRight' && dir.x !== -1) dir = { x: 1, y: 0 }; });
  if (canvas) { canvas.width = 360; canvas.height = 360; grid = 18; initSnake(); drawSnake(); }

  /* CALCULATOR */
  const calcDisplay = $('#calcDisplay'), calcGrid = document.querySelector('#m-calc .calc-grid');
  const buttons = ['7', '8', '9', '/', '4', '5', '6', '*', '1', '2', '3', '-', '0', '.', '=', '+'];
  function makeCalc() {
    if (!calcGrid) return;
    calcGrid.innerHTML = '';
    buttons.forEach(b => {
      const btn = document.createElement('button'); btn.className = 'calc-btn'; if ('+-*/'.includes(b)) btn.classList.add('op'); btn.textContent = b; btn.dataset.val = b; calcGrid.appendChild(btn);
      btn.addEventListener('click', () => handleCalc(b));
    });
    const clear = document.createElement('button'); clear.className = 'calc-btn ghost'; clear.textContent = 'C'; calcGrid.appendChild(clear); clear.addEventListener('click', () => { expr = ''; updateCalc(); });
    const del = document.createElement('button'); del.className = 'calc-btn ghost'; del.textContent = '⌫'; calcGrid.appendChild(del); del.addEventListener('click', () => { expr = expr.slice(0, -1); updateCalc(); });
  }
  let expr = '';
  function updateCalc() { if (calcDisplay) calcDisplay.value = expr || '0'; }
  function handleCalc(key) { if (key === '=') { try { expr = String(safeEval(expr)); } catch (e) { expr = 'ERR'; } updateCalc(); return; } expr += key; updateCalc(); }
